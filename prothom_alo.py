...
# session = HTMLSession()
res = session.get('https://www.prothomalo.com/collection/latest/') #link

questions = res.html.find('.bn-story-card') #HTML parent id (by '.' catch the html class)

for summary in questions:
    question = summary.find('.customStoryCard9-m__wrapper__yEFJV > a', first=True)  #Question Headline HTML class id
